...
module.exports = {
    ...
    getById(req, res, next) {
        ...
        .then((uniquerole) => {
            if (!uniquerole) {
            return res.status(httpStatus.NOT_FOUND).send({message:'Un tel role n\'existe pas!'})
            }
    ...
    create(roledata) {
        ...
        console.log('Erreur : status "',httpStatus.BAD_REQUEST,'" - Quelque chose ne va pas dans la création du role');
        return res.status(httpStatus.BAD_REQUEST).send('Something wrong in New role');  
        })
    },
