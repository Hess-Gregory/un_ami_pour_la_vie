...
const paramValidation = {
    updateUser: {
        body: {
            ...
            birthday: Joi.date(),
            sexGenre: Joi.string(),
            adressbook: Joi.number().min(0).max(1),
            adPvStreet: Joi.string(),
            adPvNum: Joi.string(),
            adPvZip:  Joi.number().min(1000).max(98999),
            adPvCity: Joi.string(),
            ...
            adProNum: Joi.string(),
            adProZip:  Joi.number().min(1000).max(98999),
...
res.locals.session = JSON.parse(Buffer.from((authorization.split(' ')[1]).split('.')[1], 'base64').toString())
    ...
    router.route('/user/:id').get(validate(paramValidation.getUser),adminController.getById); // GET /api/admins/user/activate/:id
